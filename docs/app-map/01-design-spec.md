...
- Automatic mode ports the Python pose rules (flirty was removed by user request; its finger-on-lips rule now triggers shush, and a visible tongue vetoes the gasp), plus eight web-only rules (symmetric smile, one-sided smirk, arms open wide, point at chest, fists with a scream, eyes closed, and two close-to-camera hand rules — sigma near the face, come-here lower — using apparent hand size as a depth proxy). Crashing out requires a genuinely wide-open mouth so dance is not stolen. Thinking stays manual-pick only. A digital zoom (1x/1.5x/2x) crops preview and recording alike; detection sees the full frame. Manual selection always works without models; selecting a meme locks it until Auto is tapped.
