...
self.addEventListener('fetch', event => {
  ...
  if (request.mode === 'navigate') {
    event.respondWith(fetch(request).then(async response => {
      if (response.ok && !response.redirected && new URL(response.url).pathname === '/' && (response.headers.get('content-type') || '').includes('text/html')) {
        const html = await response.clone().text();
        if (html.includes('id="root"') && html.includes('Meme camera')) { const cache = await caches.open(CACHE); await cache.put('/', response.clone()); }
      }
      return response;
    }).catch(async () => (await caches.match('/')) || new Response('Mở app khi có mạng để tải lần đầu.', { status: 503, headers: { 'Content-Type': 'text/plain; charset=utf-8' } })));
    return;
  }
  if (!STATIC.test(url.pathname) || url.pathname === '/sw.js') return;
