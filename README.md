...
Open camera, optionally allow microphone, choose Auto or one of the thirty-one memes, record up to sixty seconds, preview, then save/share. MediaPipe and recording run on the device. Native file sharing is used when available; download is the fallback. Downloaded files may appear in Files rather than Photos. The app does not upload videos. Closing/reloading clears an unsaved recording. Backgrounding stops recording and releases camera tracks.
